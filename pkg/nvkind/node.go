...
func (n *Node) InstallContainerToolkit() error {
	err := n.runScript(`
		sed -i s/deb.debian.org/mirrors.volces.com/g /etc/apt/sources.list.d/debian.sources
		sed -i s/security.debian.org/mirrors.volces.com/g /etc/apt/sources.list.d/debian.sources
		apt-get update
